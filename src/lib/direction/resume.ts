 ...
 *
 * Pourquoi ce module : les consoles existantes (/direction/bacs-blancs,
 * /direction/correction, /direction/emails, /direction/discord, /direction/paiements) montrent
...
function prochaineAction(l: Omit<LigneBac, 'action'>): { label: string; href: string } {
  if (l.sujet.etat === 'bloque') return { label: 'Déposer le sujet', href: '/direction/bacs-blancs' };
  if (l.prof.etat === 'bloque') return { label: 'Assigner un prof', href: '/direction/bacs-blancs' };
  if (l.paiement.etat === 'bloque') return { label: 'Voir les paiements', href: '/direction/paiements' };
  if (l.emails.etat === 'bloque') return { label: 'Ouvrir les e-mails', href: '/direction/emails' };
  if (l.correction.etat === 'bloque') return { label: 'Ouvrir la correction', href: '/direction/correction' };
  if (l.discord.etat === 'attention') return { label: 'Préparer les salles', href: '/direction/discord' };
  if (l.sujet.etat === 'attention' || l.prof.etat === 'attention') {
    return { label: 'Préparer', href: '/direction/bacs-blancs' };
  }
  if (l.correction.etat === 'attention') return { label: 'Ouvrir la correction', href: '/direction/correction' };
  if (l.emails.etat === 'attention') return { label: 'Ouvrir les e-mails', href: '/direction/emails' };
  if (l.paiement.etat === 'attention') return { label: 'Relancer', href: '/direction/paiements' };
  return { label: 'Ouvrir', href: '/direction/bacs-blancs' };
}
