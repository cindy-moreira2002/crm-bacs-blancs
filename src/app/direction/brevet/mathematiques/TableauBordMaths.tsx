...
/**
 * Tableau de bord des MATHÉMATIQUES au brevet.
 *
 * Écran propre à la matière : il connaît ses deux parties (6 et 14), le fait
 * que les 2 points de qualité rédactionnelle sont COMPRIS dans les 14, et que
 * la calculatrice est interdite en partie 1. Il n'affiche jamais un examen de
 * français ni un bac blanc.
 */
...
export function TableauBordMaths() {
  ...
  const charger = useCallback(async () => {
    try {
      const [rE, rS] = await Promise.all([
        fetch('/api/admin/brevet/mathematiques'),
        fetch('/api/admin/brevet/mathematiques/statistiques'),
      ]);
  ...
  async function creer(form: FormData) {
    setEnCours(true);
    try {
      const r = await fetch('/api/admin/brevet/mathematiques', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-6xl mx-auto space-y-6">
        <EnteteBrevet
          ...
          titre="Mathématiques — brevet blanc"
          fil={[{ href: '/direction/brevet', texte: 'Brevet' }]}
          soustitre={
            <>
              <p>
                L’épreuve dure 2 heures et vaut <strong>20 points</strong> : partie 1{' '}
                <strong>Automatismes 6 points</strong> sans calculatrice, partie 2{' '}
                <strong>Raisonnement et résolution de problèmes 14 points</strong>. Les{' '}
                <strong>2 points de qualité de la rédaction sont compris dans les 14</strong> — un
                barème qui les ajouterait au-dessus est refusé.
              </p>
              <p className="text-sm text-gray-600 mt-2">
                Les essais et les démarches engagées, même non abouties, doivent être pris en compte
                (note de service NOR MENE2515977N, BO n° 33 du 4 septembre 2025).
              </p>
            </>
          }
          actions={
            <>
              <Link href="/direction/brevet/mathematiques/copies">
        ...
        {ouvert && (
          <Carte
            titre="Créer un brevet blanc de mathématiques"
            aide="Le barème 1.0 naît avec lui : deux parties à 6 et 14, et les huit points de contrôle de la qualité rédactionnelle, déjà compris dans les 14."
          >
            <form action={creer} className="grid sm:grid-cols-2 gap-4">
              <Champ label="Identifiant" aide="Stable, il ne se renomme plus ensuite.">
                <input name="code" required placeholder="brevet_maths_2027_01" className={classeInput} />
              </Champ>
              <Champ label="Titre">
                <input name="titre" required placeholder="Brevet blanc de mathématiques — janvier" className={classeInput} />
              </Champ>
              <Champ label="Session">
        ...
        {stats && stats.copies.total > 0 && (
          <Carte
            titre="Statistiques des mathématiques"
            aide="Elles ne sont jamais mêlées à celles du français ni à celles du baccalauréat."
          >
            <div className="grid sm:grid-cols-4 gap-4 mb-5">
              <Chiffre libelle="Copies corrigées" valeur={stats.copies.corrigees} />
              <Chiffre
                libelle="En attente de validation"
                valeur={stats.copies.en_validation}
                alerte={stats.copies.en_validation > 0}
              />
            ...
            {stats.parties.length > 0 && (
              <div className="space-y-3 mb-5">
                <p className="text-sm font-semibold text-gray-800">Réussite moyenne par partie</p>
                {stats.parties.map((p) => (
                  <JaugeBloc key={p.code} libelle={p.libelle} saisi={p.moyenne ?? 0} attendu={p.max || 1} />
                ))}
              </div>
            )}
        ...
        {examens === null ? (
          ...
          <Carte>
            <p className="text-gray-700 font-medium">Aucun brevet blanc de mathématiques n’existe encore.</p>
            <p className="text-sm text-gray-500 mt-2">
              Crée-en un, colle le sujet et son corrigé, puis saisis les automatismes et les
              questions de la partie 2, étape valorisable par étape valorisable.
            </p>
          </Carte>
        ) : (
...
function CarteExamen({ examen }: { examen: Examen }) {
  ...
  return (
    <Link
      href={`/direction/brevet/mathematiques/${examen.id}`}
      className="block bg-white rounded-2xl border border-gray-200 hover:border-cyan-400 shadow-sm p-5 transition"
    >
