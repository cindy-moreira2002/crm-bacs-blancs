...
/**
 * Faire entrer quelqu'un dans la direction.
 *
 * On ne crée pas son mot de passe à sa place : on fabrique un lien à usage
 * unique, valable 72 h, qu'on lui envoie. Il choisit son mot de passe, et son
 * compte naît avec l'accès direction. C'est aussi la seule façon d'ouvrir un
 * accès à quelqu'un qui n'est pas encore dans la liste.
 */
function InvitationDirection() {
  ...
  const fabriquer = async (e: React.FormEvent) => {
    e.preventDefault();
    setOccupe(true);
    setErreur(null);
    setLien(null);
    try {
      const r = await fetch('/api/direction/invitation', {
        method: 'POST',
      ...
      if (!r.ok) {
        setErreur(data.error ?? 'Lien impossible à fabriquer.');
        return;
      }
      setLien(data.lien);
    } catch {
  ...
  if (!ouvert) {
    return (
      <button
        ...
        className="rounded-xl border border-purple-300 bg-purple-50 px-4 py-2.5 text-sm font-semibold text-purple-800"
      >
        🧭 Inviter quelqu’un dans la direction
      </button>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-purple-200 bg-white p-4">
      <p className="font-semibold text-slate-900">🧭 Inviter dans la direction</p>
      <p className="mt-1 text-xs text-slate-500">
        La personne recevra un lien valable 72 h : elle y choisit son propre mot de passe, et entre
        dans l’espace direction. Elle voit et peut tout — bacs blancs, e-mails, paiements, comptes.
      </p>

      <form onSubmit={fabriquer} className="mt-3 flex flex-wrap gap-2">
        <input
          type="email"
          required
          value={email}
        ...
        >
          {occupe ? 'Création…' : 'Fabriquer le lien'}
        </button>
        <button
          type="button"
          onClick={() => setOuvert(false)}
          className="px-2 text-sm text-slate-400 underline"
        >
          Fermer
        </button>
      </form>
      ...
      {lien && (
        <div className="mt-3 rounded-xl bg-slate-50 p-3">
          <p className="text-xs font-semibold text-slate-700">
            À envoyer à {email} (par message, pas en public) :
          </p>
          <p className="mt-1 break-all font-mono text-[11px] text-slate-600">{lien}</p>
          <button
            onClick={async () => {
                            ...
                            <div className="grid md:grid-cols-3 gap-6">
                              ...
                              <div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  {/* L'espace Direction est partagé : on y entre
                                      avec SON compte, jamais avec celui d'une
                                      autre. Donner l'accès, c'est cocher ici —
                                      il n'y a pas d'autre porte. */}
                                  <button
                                    onClick={() =>
                                      modifier(
                                        p.id,
                                        { role: p.role === 'admin' ? 'prof' : 'admin' },
                                        p.role === 'admin'
                                          ? `${p.prenom} n’a plus accès à la direction.`
                                          : `${p.prenom} entre dans la direction (tout le pilotage).`,
                                      )
                                    }
                                    disabled={occupe === p.id || p.id === monId}
                                    className={`w-full px-3 py-1.5 rounded-lg text-xs font-semibold disabled:opacity-40 ${
                                      p.role === 'admin'
                                  ...
                                  >
                                    {p.role === 'admin'
                                      ? '🧭 Retirer l’accès direction'
                                      : '🧭 Donner l’accès direction'}
                                  </button>
                                  {p.role === 'admin' && p.id !== monId && (
                                    <p className="text-[11px] text-slate-500">
                                      Voit et peut tout : bacs blancs, e-mails, paiements, comptes.
                                    </p>
                                  )}
                                  <button
                                    onClick={() =>
